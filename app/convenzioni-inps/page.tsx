...
import { CONVENZIONI_INPS, type ConvenzioneInps } from "@/lib/convenzioni-data";

/**
 * app/convenzioni-inps/page.tsx
 *
 ...
 *
 * Stessa struttura a due colonne di app/patti-federativi/page.tsx: a
 * sinistra un accordion con una convenzione per voce, a destra un
 * pannello a tab che mostra l'anteprima della convenzione selezionata —
 * sincronizzati tra loro. Click sull'anteprima (o "Sfoglia il documento")
 * apre il lightbox con tutte le pagine scansionate. Nessun download.
 */
...
const Breadcrumb = () => (
  <nav aria-label="breadcrumb" className={cn(MONO, "flex items-center gap-2 text-[11px] uppercase tracking-[0.2em]")}>
    ...
    <span style={{ color: NAVY }}>Convenzioni INPS</span>
  </nav>
);

// ─── Accordion voce — una convenzione per riga, sincronizzata col pannello ─
const ConvenzioneAccordionItem = ({
  convenzione,
  index,
  active,
  onSelect,
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <span className={cn(MONO, "block text-[10px] uppercase tracking-[0.15em]")} style={{ color: INK_MUTED }}>
              {convenzione.partner} — {convenzione.date}
            </span>
            <span
        ...
        {active && (
          ...
          >
            <div className="pb-6 pl-5">
              ...
              {convenzione.previewImages.length > 0 && (
                <button
                  type="button"
                  onClick={onPreview}
                  className={cn(
...
// ─── Pannello destro: anteprima della convenzione attiva + tab in basso ──
const ConvenzionePanel = ({
  attiva,
  onOpenLightbox,
}: {
  attiva: ConvenzioneInps;
  onOpenLightbox: () => void;
}) => {
  const [errored, setErrored] = useState(false);
  useEffect(() => setErrored(false), [attiva.slug]);
  const hasPreview = attiva.previewImages.length > 0 && !errored;
        ...
        <AnimatePresence mode="wait">
          {hasPreview ? (
            ...
            >
              ...
              >
                {attiva.previewImages.length} pagine
              </span>
              <span
                className={cn(
                  MONO,
...
const PreviewLightbox = ({
  ...
  convenzione: ConvenzioneInps | null;
  onClose: () => void;
}) => {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight") setPage((p) => Math.min(p + 1, convenzione.previewImages.length - 1));
  ...
  if (!convenzione) return null;
  const total = convenzione.previewImages.length;
          ...
          <div className="relative flex-1 overflow-hidden rounded-xl bg-white">
            <AnimatePresence mode="wait">
              ...
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={convenzione.previewImages[page]}
                  alt={`${convenzione.title} — pagina ${page + 1}`}
...
// ─── Page ───────────────────────────────────────────────────────────────────
export default function ConvenzioniInpsPage() {
  const [activeSlug, setActiveSlug] = useState(CONVENZIONI_INPS[0]?.slug ?? "");
  const [lightboxConvenzione, setLightboxConvenzione] = useState<ConvenzioneInps | null>(null);

  const attiva = useMemo(
    () => CONVENZIONI_INPS.find((c) => c.slug === activeSlug) ?? CONVENZIONI_INPS[0],
    [activeSlug]
  );

  const totalPages = useMemo(
    () => CONVENZIONI_INPS.reduce((sum, c) => sum + c.previewImages.length, 0),
    []
  );
    ...
    return (
      <main className="bg-white overflow-x-hidden">
        <section className="mx-auto max-w-3xl px-6 py-24 text-center">
          <p style={{ color: INK_MUTED }}>Nessuna convenzione disponibile al momento.</p>
        </section>
      </main>
    );
  }
    ...
    <main className="relative w-full overflow-x-hidden bg-white">
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto grid max-w-7xl grid-cols-1 gap-12 px-6 py-16 sm:py-20 md:grid-cols-12 lg:gap-14">
          ...
          <div className="md:col-span-6">
            ...
            <FadeUp delay={0.14} className="mt-7 max-w-md">
              <p className="text-base leading-relaxed" style={{ color: INK_MUTED }}>
                ...
                <strong style={{ color: NAVY }}>INPS ed ES.A.AR.CO.</strong>{" "}
                regolano la riscossione dei contributi di assistenza
                contrattuale e i rapporti tra l&apos;Istituto e la
                Confederazione, a beneficio delle imprese e dei lavoratori
                associati.
              </p>
            </FadeUp>
            ...
            <FadeUp delay={0.2} className="mt-6 flex flex-wrap gap-2">
              {[
                `${CONVENZIONI_INPS.length} convenzioni sottoscritte`,
                ...(totalPages > 0 ? [`${totalPages} pagine consultabili`] : []),
            ...
            <FadeUp delay={0.26} className="mt-10">
              <div style={{ borderTop: `1px solid ${HAIRLINE}` }}>
                {CONVENZIONI_INPS.map((c, i) => (
                  <ConvenzioneAccordionItem
                    key={c.slug}
                    convenzione={c}
                    index={i}
                    active={c.slug === activeSlug}
                    onSelect={() => setActiveSlug(c.slug)}
                    onPreview={() => setLightboxConvenzione(c)}
                  />
                ))}
              </div>
            </FadeUp>
          ...
          <div className="md:col-span-6">
            <FadeUp delay={0.16}>
              <ConvenzionePanel attiva={attiva} onOpenLightbox={() => setLightboxConvenzione(attiva)} />
              <ConvenzioneTabList activeSlug={activeSlug} onSelect={setActiveSlug} />
            </FadeUp>
          </div>
        </div>
      </section>

      <PreviewLightbox convenzione={lightboxConvenzione} onClose={() => setLightboxConvenzione(null)} />
    </main>
  );
}