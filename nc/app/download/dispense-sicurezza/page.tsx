 ...
 *
 * Stessa identità visiva delle altre pagine (vedi globals.css / dispense/page.tsx):
...
const Breadcrumb = () => (
  <nav aria-label="breadcrumb" className={cn(MONO, "flex items-center gap-2 text-[11px] uppercase tracking-[0.2em]")}>
    ...
    <Link href="/dispense" className="transition-colors" style={{ color: INK_MUTED }}>
      Dispense
    </Link>
    <span style={{ color: HAIRLINE }}>/</span>
    <span style={{ color: NAVY }}>Sicurezza sul Lavoro</span>
  </nav>
);
    ...
    <div className="relative mx-auto max-w-7xl px-6">
      ...
      <FadeUp delay={0.16} className="mt-9 max-w-3xl space-y-5">
        <p className="text-base sm:text-lg leading-relaxed" style={{ color: INK_MUTED }}>
          Qui trovi le dispense dedicate ai <strong style={{ color: NAVY }}>corsi di sicurezza sul lavoro</strong>:
          formazione obbligatoria, aggiornamenti e materiali tecnici per lavoratori, preposti e figure specializzate.
        </p>
        <p className="text-base sm:text-lg leading-relaxed" style={{ color: INK_MUTED }}>
          Scegli il corso che ti interessa e scarica il PDF corrispondente.
        </p>
      </FadeUp>
    </div>
  </section>
      ...
      <FadeUp className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className={cn(DISPLAY, "mt-2 text-2xl sm:text-3xl font-medium leading-tight")} style={{ color: NAVY }}>
            Tutte le dispense sicurezza
          </h2>
        </div>
        <span className="text-sm" style={{ color: INK_MUTED }}>
          {DISPENSE_SICUREZZA.length} dispense disponibili
        </span>
      </FadeUp>

      <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {DISPENSE_SICUREZZA.map((d, i) => {
...
export default function DispenseSicurezzaPage() {
  return (
