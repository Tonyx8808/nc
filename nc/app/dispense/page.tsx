 ...
 *
 * Versione senza login: i PDF stanno in public/dispense/ e il download è
 * diretto. Il controllo email + OTP si aggiunge in un secondo momento.
 */
...
const Breadcrumb = () => (
  <nav aria-label="breadcrumb" className={cn(MONO, "flex items-center gap-2 text-[11px] uppercase tracking-[0.2em]")}>
    ...
    <span style={{ color: NAVY }}>Dispense</span>
  </nav>
);

// ─── Header descrittivo ─────────────────────────────────────────────────────
const DispenseHeader = () => (
    ...
    <div className="relative mx-auto max-w-7xl px-6">
      ...
      <FadeUp delay={0.16} className="mt-9 max-w-3xl space-y-5">
        <p className="text-base sm:text-lg leading-relaxed" style={{ color: INK_MUTED }}>
          In questa sezione trovi il materiale didattico dei nostri{" "}
          <strong style={{ color: NAVY }}>percorsi formativi</strong>: dispense
          teoriche, video lezioni e guide operative pensate per accompagnare
          allievi e professionisti nello studio, dai profili socio-assistenziali
          alle competenze digitali di base.
        </p>
        <p className="text-base sm:text-lg leading-relaxed" style={{ color: INK_MUTED }}>
          Scegli la materia che ti interessa e scarica il PDF corrispondente.
        </p>
      </FadeUp>
    </div>
  </section>
    ...
    <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="flex flex-col items-start gap-3 sm:items-end">
          <span className="text-sm" style={{ color: INK_MUTED }}>
            {DISPENSE.length} dispense disponibili
          </span>
          <Link
            href="/download/dispense-sicurezza"
            className={cn(
              MONO,
              "inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-[11px] uppercase tracking-[0.15em] text-white transition-transform hover:scale-[1.03]"
            )}
            style={{ backgroundColor: RED }}
          >
            Dispense Sicurezza sul Lavoro
          </Link>
        </div>
      </FadeUp>
